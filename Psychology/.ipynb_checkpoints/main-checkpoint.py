...
if __name__ == '__main__':
    ...
    val_dataset = FacesMemorabilityDataset(dataframe=val_df, root_dir=root_dir, target=args.target, transform=test_transform)
    ...
    # 4. Training
    results_df = pd.DataFrame(columns=['Epoch', 'Pearson Correlation'])
    best_pearson_corr = 0.0
    best_epoch = 0
    epochs_no_improve = 0  
    n_epochs_stop = args.es
    for epoch in range(1, args.epochs + 1):
        ...
        current_pearson_corr = val(val_loader, model, device)
        if current_pearson_corr > best_pearson_corr:
            best_pearson_corr = current_pearson_corr
            best_epoch = epoch
            epochs_no_improve = 0
            torch.save(model.state_dict(), best_model_path)
            print(f"New best model saved at epoch {epoch} with Pearson Correlation: {best_pearson_corr}")
            state_dict = model.state_dict()
            # torch.save(state_dict, str(dir_checkpoint / 'checkpoint_epoch{}.pth'.format(epoch)))
            # logging.info(f'Checkpoint {epoch} saved!')
        else:
            epochs_no_improve += 1
        trainset_corr = val(train_loader, model, device)
        print(
            f"Epoch: {epoch}, \n"
            f"Pearson Correlation of Train Set: {trainset_corr}, \n"
            f"Current Pearson Correlation of Validate Set: {current_pearson_corr}, \n"
            f"Best Pearson Correlation of Validate Set: {best_pearson_corr} (Best Epoch {best_epoch})"
        )

        new_row = pd.DataFrame({'Epoch': [epoch], 'Pearson Correlation': [current_pearson_corr]})
        results_df = pd.concat([results_df, new_row], ignore_index=True)
        if epochs_no_improve == n_epochs_stop:
            print(f"Early stopping triggered after {n_epochs_stop} epochs without improvement.")
            break 
    model.load_state_dict(torch.load(best_model_path))
    print("Evaluating the best model on the test set.")
    
    test_result = test(test_loader, model, device)
    test_result = round(test_result, 2)
    # saved_files = f'{args.model}_{args.target}_epoch_pearson_correlation.xlsx'
    saved_model_path = f'{dir_checkpoint}/{args.target}_best_model_{test_result}.pth'
    torch.save(model.state_dict(), saved_model_path)
    
    # results_df.to_excel(saved_files, index=False)
